...
def resolve_card_weighting(value: object) -> str:
    ...
    return weighting if weighting in {"sqrt", "heavy", "presence", "binary_idf", "counts_idf", "raw"} else "sqrt"


def weighted_card_matrix(matrix: pd.DataFrame, weighting: str) -> pd.DataFrame:
    presence = (matrix > 0).astype(float)
    if weighting in {"binary_idf", "counts_idf"}:
    ...
    if weighting == "counts_idf":
        return matrix.mul(idf, axis=1)
    if weighting == "heavy":
